...
export class SidebarComponent implements OnInit {
  ...
  filterMenuByRole(menuItems: MenuItem[], userRole: number): MenuItem[] {
    return menuItems.filter(item => {
      if (item.rolesAllowed && !item.rolesAllowed.includes(userRole)) {
        // Si el ítem tiene roles permitidos y el usuario no tiene ese rol, se oculta
        return false;
      }

      if (item.subItems) {
        // Si tiene sub-items, se aplica la misma lógica de filtrado
        item.subItems = this.filterMenuByRole(item.subItems, userRole);
      }

      return true; // Mostrar el ítem si pasa la verificación del rol
    });
  }
  toggleItem(item: any) {
